...
<div class="container wrapper">      
    ...
    foreach ($applications as $key => $value) { 
        ...
        $reference_data = $this->Custom->find_reference_data($app_id);
                ...
                foreach($visa_process1 as $key=>$process){
                   ...
                   // pr($process_statuses);die;
                    ...
                    <div class="<?php echo $main_div_class;?>" id="main_<?php echo $value['Application']['id']."_".$process_counter;?>">
                        ...
                        <p class="app_reference_date" id="date_<?php echo $app_id.'_'.$process_counter?>">
                        <?php echo $typing_date;?></p>
                    </div>
                <?php 
                $process_counter++;
                }
    ...
    <?php } ?>
    <hr>
    <?php if($application_count>= PAGE_LIMIT)
    { ?>
        <div id="pgBox">
        <div id="paging">
...
<div id="myModal" class="modal">
  ...
  <div class="modal-content">
    ...
    <div class="col-sm-16">
        <?php echo $this->Form->create('ApplicationProcess', array('url'=>array('controller'=>'applications','action'=>'visa_process_feedback_hr') ,'method' => 'post', "type" => "file",'id'=>"jsonFormFeedback"));  
...
$('.myBtnWhat').click(function(){
    ...
    var values = "application_id="+application_id+"&process_stage="+process_stage;
    $.ajax({
          url: "<?php echo SITE_URL?>applications/get_issue_detail",
          type: "post",
          data: values,
          success: function(res){
            $('#what_tag_options').html(res);
          }
      });
        ...
        var process_stage = $(this).attr('process_stage');
         $("#jsonFormFeedback").ajaxForm({
            success: function(response){ alert(response); 
               $('#what_'+application_id+"_"+process_stage).removeClass('myBtnWhat');
               $('#what_'+application_id+"_"+process_stage).html('');
               $('#main_'+application_id+"_"+process_stage).removeClass('process-button-red');
               $('#main_'+application_id+"_"+process_stage).addClass('process-button-cyan');
               modalWhat.style.display = "none";
            }
        }).submit();
    });
