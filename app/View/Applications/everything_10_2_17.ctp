        ...
        <div class="process-type">
         <span class="divs<?php echo $app_id?>">
            ...
            if($process_type==NEW_RESIDENCY)
            {
                ...
                $new_residency_process[1] = "Entry Permit ".$entry_permit_type;
                $visa_process = $new_residency_process;

                $outpass_type = unserialize(OUTPASS_TYPE);
                $new_residency_process[2] = $outpass_type[$country_type];
                $visa_process = $new_residency_process;
                
            }elseif($process_type==RENEW_RESIDENCY)
            {
                $visa_process = $renew_residency_process;
            }elseif($process_type==LOCAL_TRANSFER)
            {
                $visa_process = $local_transfer_process;
            }elseif($process_type==CANCELLATION)
            {
                $visa_process = $cancellation_process;
            }
                ...
                foreach($visa_process1 as $key=>$process){
                   ...
                   // pr($process_statuses);die;
                    ...
                    <div class="<?php echo $main_div_class;?>" id="main_<?php echo $value['Application']['id']."_".$process_counter;?>">
                        ...
                        <!-- <span class="img-question myBtnWhat" style="position: absolute;">?</span>

                        <span class="put_reference" title="Put reference number" style="margin-left:40px;"><img src="<?php echo SITE_URL.'img/checkbox_uncheck.png'; ?>" height="14px"></span>

                        <span style="float:right; margin-right:10px;"><img src="<?php echo SITE_URL.'img/'.$chckbox_icon; ?>" height="14px"></span> -->
            ...
            <div class="process-button1">
                ...
                <span><a href="<?php echo SITE_URL.'applications/barcode_print/'.$value['Application']['barcode'].'/'.$value['Company']['company_name'];?>" target='_blank'   ><img src="<?php echo SITE_URL.'img/barcode-icon.png'; ?>" alt="Barcode" height="26"/></a></span>

                <!-- class="barcode_print" -->
                <!-- <span><a href="javascript:void(0)" app_id="<?php echo $app_id; ?>" class="barcode_print" barcode="<?php echo $value['Application']['barcode']?>" company_name="<?php echo $value['Company']['company_name']?>"><img src="<?php echo SITE_URL.'img/barcode-icon.png'; ?>" alt="Barcode" height="26"/></a></span> -->
...
<!--====================== POPUP START print barcode====================-->
<div id="myModalPrintBarcode" class="modal">
  <div class="modal-content">
    <span class="close">×</span>
    <h4 id="head_txtx">Print Barcode</h4>
     <div class="col-sm-16" id="barcode_show"></div> 
    <div id="document_list"></div>
    <a href="javascript:void(0)" onclick="printFunction()" id="hide_other_section">Print</a>
</div>
</div>

<style type="text/css" media="print">
    #hide_other_section, .container, .wrapper, .close, #head_txtx, #myModalPrintBarcode{ display:none; } 
    @page {
        margin: 0mm;  /* this affects the margin in the printer settings */  
    }
    .printTxtLeft{ width:48% !important;}
    .printTxtRight{ width:48% !important; float:right;}
</style>
<!--====================== POPUP END====================-->
...
//==========Popup for print barcode========
var modalPrintBarcode = document.getElementById('myModalPrintBarcode');
$('.barcode_print').click(function(){
    $barcode = $(this).attr("barcode");
    $company_name = $(this).attr("company_name");
    $('#barcode_show').html('');
    
    var values = "barcode="+$barcode+"&company_name="+$company_name;
    
    $.ajax({
          url: "<?php echo SITE_URL?>applications/get_barcode_image",
          type: "post",
          data: values,
          success: function(result){
            $('#barcode_show').html(result);
          }
    }); 

    modalPrintBarcode.style.display = "block";
});
function printFunction() {
    window.print();
}
//==========END Popup for print barcode========
...
$('.close').click(function(){
    modalPrintBarcode.style.display = "none";
});
...
window.onclick = function(event) {
    ...
    if (event.target == modalPrintBarcode) {
        modalPrintBarcode.style.display = "none";
    }
